...
public class ServiceFuncion implements InterfaceServiceFuncion {
  ...
  public Funcion addFuncion(Funcion f) {
    // f.setHoraFin(f.getHoraInicio().plusMinutes(f.getPelicula().getDuracionMin() +
    // 30));
    ...
    if(!isPeliculaEnCartelera(f.getPelicula().getIdPelicula())){
      peliculaService.cambioPeliculaToCartelera(f.getPelicula().getIdPelicula());
    }
    return repositoriofuncion.save(f);
  }

  private boolean isPeliculaEnCartelera (Integer idPelicula){
    Pelicula p = peliculaService.findPeliculaPorId(idPelicula);
    System.out.println("la pelicula esta esta en cartelera: " + p.isCartelera());
    return p.isCartelera();
  }
  ...
  public FuncionAltaDTO getFuncionEdicion(Integer id) {
    ...
    FuncionAltaDTO funcionE = new FuncionAltaDTO(f.getIdFuncion(),
        f.getSala(),
        f.getPelicula(),
